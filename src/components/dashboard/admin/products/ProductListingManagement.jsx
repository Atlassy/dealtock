...
const ProductListingManagement = () => {
  ...
  /**
   * Load active generic decline reasons.
   *
   * We intentionally read the reason catalogue from the database instead
   * of hard-coding the complete catalogue in the frontend.
   */
  const loadDeclineReasons = useCallback(async () => {
    ...
    /*
     * Generic seller products use the generic moderation context.
     * Returned-product-specific reasons must not appear here.
     */
    setDeclineReasons(
      (data || []).filter(
        (reason) =>
          !reason.context ||
          reason.context === "generic"
      )
    );
  }, []);

  /**
   * Load products through the admin_products view.
   *
   * IMPORTANT:
   * This screen manages normal product listings.
   * Returned products remain managed by ReturnedProductsQueue.
   */
  const loadProducts = useCallback(async () => {
    try {
      ...
      toast.error(
        `Failed to load product listings: ${error.message}`
      );
    } finally {
      setLoading(false);
    }
  ...
  /**
   * Execute the DB-controlled moderation transition.
   *
   * NO direct UPDATE of products.listing_status is allowed here.
   */
  ...
  const handleApprove = async (product) => {
    try {
      ...
      toast.error(
        `Unable to approve "${product.name}": ${error.message}`
      );
    } finally {
  ...
  const handleDecline = async () => {
    ...
    try {
      ...
      toast.error(
        `Unable to decline "${declineTarget.name}": ${error.message}`
      );
    } finally {
      setProcessingId(null);
    }
  ...
  const handleSuspend = async (product) => {
    if (
      !window.confirm(
        `Suspend "${product.name}" from the marketplace?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      toast.error(
        `Unable to suspend "${product.name}": ${error.message}`
      );
    } finally {
      setProcessingId(null);
    }
  ...
  const handleRestore = async (product) => {
    try {
      ...
      toast.error(
        `Unable to restore "${product.name}": ${error.message}`
      );
    } finally {
      setProcessingId(null);
    }
  ...
  /**
   * Admin reopen is deliberately NOT implemented through
   * moderate_product because the backend provides a dedicated
   * RPC for the admin override:
   *
  ...
  const handleAdminReopen = async () => {
    ...
    try {
      ...
      toast.success(
        `"${reopenTarget.name}" returned to Pending Review`
      );
      ...
      toast.error(
        `Unable to reopen "${reopenTarget.name}": ${error.message}`
      );
    } finally {
      setProcessingId(null);
    }
   ...
   * Bulk moderation.
   *
   * The backend performs the actual FSM validation per product.
   * Therefore one invalid product does not silently bypass the FSM.
   */
  const handleBulkModerate = async (newStatus) => {
    ...
    if (
      !window.confirm(
        `${actionLabel === "approve" ? "Approve" : "Suspend"} ${
          selectedIds.length
        } selected product(s)?`
      )
    ) {
    ...
    try {
      ...
      if (failures.length > 0) {
        ...
        toast.success(
          `${successes.length} product(s) updated successfully`
        );
      }
  ...
  const [bulkDeclineModalOpen, setBulkDeclineModalOpen] =
    useState(false);
  ...
  const handleBulkDecline = async () => {
    ...
    try {
      ...
      if (failures.length) {
        toast.warning(
          `${successes.length} declined, ${failures.length} failed.`
        );
        console.warn("Bulk decline failures:", failures);
      } else {
        toast.success(
          `${successes.length} product(s) declined`
        );
      }
      ...
      toast.error(
        `Bulk decline failed: ${error.message}`
      );
    } finally {
      setBulkProcessing(false);
    }
          ...
          <div className="flex flex-wrap gap-2">
            {filter === STATUS.PENDING && (
              <button
                ...
                onClick={() =>
                  handleBulkModerate(STATUS.AVAILABLE)
                }
            ...
            {filter === STATUS.AVAILABLE && (
              <button
                ...
                disabled={bulkProcessing}
                onClick={() =>
                  handleBulkModerate(STATUS.SUSPENDED)
                }
      ...
      {loading ? (
        ...
        <div className="py-16 text-center rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
          ...
          <p className="text-sm text-gray-400 mt-1">
            There are no product listings matching this view.
          </p>
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filteredProducts.map((product) => {
                  const config =
                    STATUS_CONFIG[product.listing_status] ||
            ...
            <div className="flex items-center justify-between p-5 border-b border-gray-200 dark:border-gray-700">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Review the listing information before applying
                  a moderation action.
                </p>
              </div>
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-3">
                    {(() => {
                      const config =
                        STATUS_CONFIG[
                          selectedProduct.listing_status
                        ] ||
                        STATUS_CONFIG[STATUS.PENDING];
            ...
            <div className="flex flex-wrap justify-end gap-2 p-5 border-t border-gray-200 dark:border-gray-700">
              ...
              {selectedProduct.listing_status ===
                ...
                <>
                  <button
                    ...
                    onClick={() =>
                      openDeclineModal(selectedProduct)
                    }
              ...
              {selectedProduct.listing_status ===
                ...
                <button
                  type="button"
                  onClick={() =>
                    openReopenModal(selectedProduct)
                  }
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={(event) =>
                    setDeclineReasonCode(event.target.value)
                  }
          ...
          <div className="w-full max-w-lg rounded-2xl bg-white dark:bg-gray-800 shadow-xl">
            ...
            <div className="p-5 space-y-4">
              ...
              <textarea
                value={declineReasonDetail}
                onChange={(event) =>
                  setDeclineReasonDetail(event.target.value)
                }
                rows={4}
                placeholder="Optional additional explanation..."
                className="w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white px-3 py-2"
