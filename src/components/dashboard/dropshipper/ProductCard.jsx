...
const ProductCard = ({ product, onPlaceOrder }) => {
  ...
  const fetchCommissionRate = async () => {
    ...
    try {
      ...
      setError(t('dropshipperProductCard.commissionLoadFailed'));
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between text-sm text-gray-500 dark:text-gray-400 mb-3">
          ...
          <div className="flex items-center">
            ...
            {product.seller_name || t('dropshipperProductCard.seller')}
          </div>
        </div>
        ...
        <div className="bg-gray-50 dark:bg-gray-900 p-3 rounded-lg mb-3">
          <div className="flex justify-between items-center">
            <span className="text-sm text-gray-600 dark:text-gray-400">{t('dropshipperProductCard.basePrice')}</span>
        ...
        {showCalculator ? (
          <div className="space-y-4">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {t('dropshipperProductCard.yourMarkup')}
                </label>
            ...
            {loadingCommission ? (
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg text-center">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">{t('dropshipperProductCard.calculatingCommission')}</p>
              </div>
            ) : error ? (
              <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-lg text-center">
                <p className="text-sm text-red-600 dark:text-red-400">{error}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{t('dropshipperProductCard.usingDefaultRate')}</p>
              </div>
            ) : (
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg space-y-3">
                <h4 className="font-semibold text-sm flex items-center gap-1 text-gray-900 dark:text-white">
                  <Info className="w-4 h-4" />
                  {t('dropshipperProductCard.profitCalculator')}
                </h4>

                <div className="space-y-2">
                  <div className="flex justify-between text-sm text-gray-900 dark:text-gray-100">
                    <span>{t('dropshipperProductCard.grossMarkup')}</span>
                  ...
                  <div className="flex justify-between text-sm border-t border-blue-200 dark:border-blue-800 pt-2 text-gray-900 dark:text-gray-100">
                    <span className="flex items-center gap-1">
                      {t('dropshipperProductCard.dealtockFee')}
                  ...
                  <div className="flex justify-between font-bold text-base border-t border-blue-200 dark:border-blue-800 pt-2 text-gray-900 dark:text-white">
                    <span>{t('dropshipperProductCard.yourNetProfit')}</span>
                  ...
                  <div className="text-xs text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 p-2 rounded mt-2">
                    <div className="flex justify-between">
                      <span>{t('dropshipperProductCard.customerPays')}</span>
                      <span className="font-medium text-gray-900 dark:text-white">{customerPrice.toFixed(2)} MAD</span>
                    </div>
                    <div className="flex justify-between">
                      <span>{t('dropshipperProductCard.plusShipping')}</span>
                      <span className="font-medium text-gray-900 dark:text-white">{t('dropshipperProductCard.shippingEstimate')}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="flex gap-2">
              ...
              >
                {t('dropshipperProductCard.cancel')}
              </button>
              <button
              ...
              >
                {loadingCommission ? (
                  <>
                    ...
                    {t('dropshipperProductCard.calculating')}
                  </>
                ) : (
                  t('dropshipperProductCard.placeOrder')
                )}
              </button>
            </div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 text-center">
              {t('dropshipperProductCard.feeHelpText', { rate: commissionRate })}
            </p>
          </div>
        ) : (
          <button
          ...
          >
            ...
            <span className="font-medium">{t('dropshipperProductCard.calculateYourProfit')}</span>
          </button>
        )}
        ...
        <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400 mt-4 pt-3 border-t border-gray-200 dark:border-gray-700">
          <span className="flex items-center gap-1">
            ...
            {t('dropshipperProductCard.stock', { count: product.quantity || 0 })}
          </span>
          <span className="flex items-center gap-1">
            <Truck className="w-3 h-3" />
            {t('dropshipperProductCard.deliveryEstimate')}
          </span>
        </div>
      </div>
    </div>
