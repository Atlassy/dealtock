  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 px-4 py-8">
      <div className="w-full max-w-sm bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 text-center">
        ...
        <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{t('auth.checkEmail.title')}</h2>

        <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">
          {t('auth.checkEmail.weSent')}
        </p>
        {email && (
          <p className="text-sm font-semibold text-blue-600 dark:text-blue-400 mb-4 break-all">{email}</p>
        )}
        ...
        {/* Steps */}
        <div className="bg-gray-50 dark:bg-gray-900 rounded-xl p-4 text-left space-y-2 mb-6">
          {[
            ...
            <div key={i} className="flex items-center gap-3">
              <div className="w-5 h-5 rounded-full bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 text-xs font-bold flex items-center justify-center flex-shrink-0">
                {i + 1}
              </div>
        ...
        {/* Resend */}
        <p className="text-xs text-gray-400 dark:text-gray-500 mb-2">{t('auth.checkEmail.didntReceive')}</p>
        <button
          onClick={handleResend}
          disabled={resending || resent || !email}
          className="w-full py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition mb-4"
        >
          {resending ? (
